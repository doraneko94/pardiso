...
/* -------------------------------------------------------------------- */
    unsafe { pardiso_handle_store(pt.as_slice_memory_order_mut().unwrap(), "", &mut error); }
    if error != 0 {
    ...
    unsafe { pardiso_handle_restore(pt_new.as_slice_memory_order_mut().unwrap(), "", &mut error); }
    if error != 0 {
        panic!("ERROR during restoring PARDISO structures from a file: {}", error);
    }
    println!("Restoring completed ... ");

    unsafe { pardiso_handle_delete("", &mut error); }
    if error != 0 {
